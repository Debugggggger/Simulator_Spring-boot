...
Fugit.prototype.setSeconds = function (value) {
    if (validHours.test(value)) {
      var _validSeconds$exec = validSeconds.exec(value),
          _validSeconds$exec2 = _slicedToArray(_validSeconds$exec, 2),
          t = _validSeconds$exec2[0],
          seconds = _validSeconds$exec2[1];

      this.$el.val(this.$el.val().replace(validSeconds, seconds)).trigger('change');
      this.$plugin.find('.fugit-seconds').text(seconds);
    }
  };
...
Fugit.prototype.getSeconds = function () {
    return Number(this.$el.val().split(':')[1]);
  };
